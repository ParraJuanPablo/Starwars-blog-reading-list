import React, { Component } from "react";
import { useState, useContext, useEffect } from "react";
import { Context } from "../store/appContext";

import { Link, useParams } from "react-router-dom";

export const CharacterCard = (props) => {
    const { store, actions } = useContext(Context);
    const [ selected, setSelected ] = useState("")


    function isFavorite() {
        console.log("Se activo")
        if (selected === "active"){
            store.favorites.pop(store.favorites.indexOf(store.characters[index]))
            setSelected("")
        }
        else {
            store.favorites.push(store.characters[index])
            console.log(store.favorites)
            setSelected("active")
        }
    }
    useEffect(() =>{
		if (selected === "active" && store.favorites.indexOf(store.characters[index]) === -1){
            isFavorite()
        }
	}
	, [store.favorites]);
    const index = props.index;
    const name = props.name;
    const gender = props.gender;
    const hairColor = props.hairColor;
    const eyeColor = props.eyeColor;
    return(
        <div className="card d-flex align-items-top">
            <img src="data:image/png;base64," className="card-img-top" alt="..." />
            <div className="card-body">
                <h3 className="card-title">{name}</h3>
                <p className="card-title">Gender: {gender}</p>
                <p className="card-title">Hair Color: {hairColor}</p>
                <p className="card-title">Eye-Color: {eyeColor}</p>
                <div className="d-flex justify-content-between">
                    <Link to={"/characterDescription/"+index} >
                        <button type="button" className="btn btn-outline-primary">Learn More!</button>
                    </ Link>
                    <button type="button" className={"btn btn-outline-warning " + selected} onClick={() => isFavorite()}><i className="fa-regular fa-heart"></i></button>
                </div>
            </div>
        </div>
    );
};